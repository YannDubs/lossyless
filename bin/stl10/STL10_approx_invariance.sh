...
experiment="STL10_approx_invariances"
notes="
**Goal**: Train and test set have different augmentations. Spec. with prob. p, a label outside the true invariance
class of is shuffled, with iVAE, predicted on features with MLP, ca. 50 runs for each config
"
...
if [ "$is_plot_only" = false ] ; then
  for val in 0 0.05 0.2 0.5
  do
    kwargs_dep="+data_feat.kwargs.dataset_kwargs.val_equivalence=[hflip,color,gray,equiv_resize_crop_$val]
