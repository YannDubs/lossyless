...
if [ "$is_plot_only" = false ] ; then
  for kwargs_dep in "data_feat.kwargs.batch_size=32,64,256,512,1024" "encoder.z_dim=32,512" "distortion.kwargs.arch_kwargs.complexity=1,2,3"  "distortion.kwargs.arch=linear,mlp,resnet" "optimizer@optimizer_feat=adam1e-3,adam3e-3,adam1e-4,sgd"  "featurizer.loss.beta=0.1,0.03,0.003,0.001" "optimizer@optimizer_coder=adam3e-3,sgd"  "optimizer@optimizer_online=adam3e-3,sgd"   "featurizer.loss.beta_anneal=geometric,linear" "rate.kwargs.is_endToEnd=False" "rate.kwargs.warmup_k_epoch=1,10"      
  do
